...
_CSI_DSR = _CSI + "6" + "n" #Reports the cursor position to the application as (as though typed at the keyboard) ESC[nmR, where n is the row and m is the column. (May not work on MS-DOS.)
...
def _CSI_STR_MAGNET(str): return "%s%s%s" % ( _CSI_UTIL_CLR_MAGENTA, str, _CSI_UTIL_RESET )
...
def _CSI_STR_MAGNET_BOLD(str): return "%s%s%s" % ( _CSI_UTIL_CLR_MAGENTA_BOLD, str, _CSI_UTIL_RESET )
